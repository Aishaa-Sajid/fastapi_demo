...
@router.post("/", response_model=profile.ProfileOut)
async def create_profile(
    ...
    return await profile_crud.create_profile(
        db, current_user.id, profile.model_dump()
    )


# ✅ Get My Profile
@router.get("/", response_model=profile.ProfileOut)
...
@router.put("/", response_model=profile.ProfileOut)
async def update_profile(
    updated_data: profile.ProfileUpdate,
    db: AsyncSession = Depends(get_pg_db),
    current_user=Depends(get_current_user),
):
    profile = await profile_crud.get_profile_by_user_id(db, current_user.id)
    ...
    return await profile_crud.update_profile(
        db, profile, updated_data.model_dump()
    )
